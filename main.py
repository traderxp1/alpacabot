...
def webhook():
    ...
    try:
        ...
        if action in ("PENDING_ENTRY","ENTRY"): r = handle_pending_entry(data)
        elif action == "ENTRY_FILLED":          r = handle_entry_filled(data)
        elif action == "EXIT":                  r = handle_exit(data)
        elif action == "UPDATE_BACKUP_SL":      r = handle_update_backup_sl(data)
        elif action == "CANCEL_PENDING":        r = handle_cancel_pending(data)
        else:                                   r = {"status": "غير معروف", "action": action}
...
def dashboard():
    ...
    cards = ""
    if not active:
        cards = '<div class="card"><p style="color:#555;text-align:center;padding:12px">لا توجد صفقات نشطة</p></div>'
    else:
        for sym in active:
            st = states[sym]
            status_txt = "🟢 صفقة مفتوحة" if st["in_trade"] else "🟡 أمر معلّق"
            scls = "green" if st["in_trade"] else "yellow"
            entry = st.get("entry_price") or "—"
            sl    = st.get("backup_sl") or "—"
            tp    = st.get("tp_price") or "—"
            risk  = round(float(entry) - float(sl), 4) if entry != "—" and sl != "—" else "—"
            reward = round(float(tp) - float(entry), 4) if tp != "—" and entry != "—" else "—"
            cards += f"""<div class="card"><h2>{sym}</h2>
<div class="row"><span class="label">الوضع</span><span class="val {scls}">{status_txt}</span></div>
<div class="row"><span class="label">سعر الدخول</span><span class="val">{entry}</span></div>
<div class="row"><span class="label">🛡️ ستوب لوز</span><span class="val red">{sl}</span></div>
<div class="row"><span class="label">🎯 تيك بروفت</span><span class="val green">{tp}</span></div>
<div class="row"><span class="label">الكمية</span><span class="val">{st.get('qty') or '—'}</span></div>
<div class="row"><span class="label">المخاطرة</span><span class="val red">{risk}</span></div>
<div class="row"><span class="label">الهدف</span><span class="val green">{reward}</span></div>
<div class="row"><span class="label">بريك ايفن</span><span class="val {'yellow' if st.get('be_active') else 'label'}">{'✅ مفعّل' if st.get('be_active') else 'غير مفعّل'}</span></div>
<div class="row"><span class="label">آخر إجراء</span><span class="val">{st.get('last_action') or '—'}</span></div>
<div class="row"><span class="label">آخر خطأ</span><span class="val red">{st.get('last_error') or '—'}</span></div>
</div>"""

    trades = load_trades(100)
    summary_html = ""
    table_html = '<p style="color:#555;padding:8px">لا توجد صفقات بعد</p>'

    if trades:
        total_pnl  = sum(float(t["pnl"] or 0) for t in trades)
        tp_trades  = [t for t in trades if t["exit_reason"] == "TP"]
        be_trades  = [t for t in trades if t["exit_reason"] in ("BE","SL_MARKET")]
        sl_trades  = [t for t in trades if t["exit_reason"] == "SL"]
        win_trades = [t for t in trades if float(t["pnl"] or 0) > 0]
        los_trades = [t for t in trades if float(t["pnl"] or 0) < 0]
        win_rate   = round(len(win_trades)/len(trades)*100, 1) if trades else 0
        avg_win    = round(sum(float(t["pnl"] or 0) for t in win_trades)/len(win_trades), 4) if win_trades else 0
        avg_loss   = round(sum(float(t["pnl"] or 0) for t in los_trades)/len(los_trades), 4) if los_trades else 0
        best       = max(trades, key=lambda t: float(t["pnl"] or 0))
        worst      = min(trades, key=lambda t: float(t["pnl"] or 0))
        pf_num     = sum(float(t["pnl"] or 0) for t in win_trades)
        pf_den     = abs(sum(float(t["pnl"] or 0) for t in los_trades))
        pf         = round(pf_num/pf_den, 2) if pf_den > 0 else "∞"
        clr        = "green" if total_pnl >= 0 else "red"

        summary_html = f"""<div class="card">
<h2>📊 ملخص الأداء ({len(trades)} صفقة)</h2>
<div class="stats-grid">
  <div class="stat"><div class="stat-val {clr}">{total_pnl:+.4f}</div><div class="stat-lbl">إجمالي P&L (USD)</div></div>
  <div class="stat"><div class="stat-val">{win_rate}%</div><div class="stat-lbl">معدل الربح</div></div>
  <div class="stat"><div class="stat-val">{pf}</div><div class="stat-lbl">Profit Factor</div></div>
  <div class="stat"><div class="stat-val green">{len(tp_trades)}</div><div class="stat-lbl">✅ تيك بروفت</div></div>
  <div class="stat"><div class="stat-val yellow">{len(be_trades)}</div><div class="stat-lbl">➡️ بريك ايفن</div></div>
  <div class="stat"><div class="stat-val red">{len(sl_trades)}</div><div class="stat-lbl">❌ ستوب لوز</div></div>
  <div class="stat"><div class="stat-val green">{avg_win:+.4f}</div><div class="stat-lbl">متوسط الربح</div></div>
  <div class="stat"><div class="stat-val red">{avg_loss:+.4f}</div><div class="stat-lbl">متوسط الخسارة</div></div>
  <div class="stat"><div class="stat-val green">{float(best['pnl'] or 0):+.4f}</div><div class="stat-lbl">أفضل صفقة</div></div>
  <div class="stat"><div class="stat-val red">{float(worst['pnl'] or 0):+.4f}</div><div class="stat-lbl">أسوأ صفقة</div></div>
</div></div>"""
        ...
        for t in trades:
            ...
            ct      = t["close_time"]
            ct_str  = ct.strftime("%m-%d %H:%M") if hasattr(ct,"strftime") else str(ct)[:16]
            entry_p = f"{float(t['entry_price']):.4f}" if t["entry_price"] else "—"
            exit_p  = f"{float(t['exit_price']):.4f}" if t["exit_price"] else "—"
            sl_p    = f"{float(t['sl_price']):.4f}" if t["sl_price"] else "—"
            tp_p    = f"{float(t['tp_price']):.4f}" if t["tp_price"] else "—"
            rr      = f"{float(t['rr_actual']):.2f}R" if t["rr_actual"] else "—"
            dur     = f"{t['duration_min']}د" if t["duration_min"] else "—"
            pct     = f"{float(t['pnl_pct']):.2f}%" if t["pnl_pct"] else "—"
            rows += f"""<tr>
<td>{ct_str}</td><td><b>{t['symbol']}</b></td><td>{reason_ar(t['exit_reason'])}</td>
<td>{entry_p}</td><td>{exit_p}</td><td class="red">{sl_p}</td><td class="green">{tp_p}</td>
<td class="{clr2}">{pnl_val:+.4f}</td><td class="{clr2}">{pct}</td><td>{rr}</td><td>{dur}</td>
</tr>"""

        table_html = f"""<div style="overflow-x:auto">
<table><tr>
<th>الوقت</th><th>رمز</th><th>النتيجة</th>
<th>دخول</th><th>خروج</th><th>SL</th><th>TP</th>
<th>P&L</th><th>%</th><th>RR</th><th>مدة</th>
</tr>{rows}</table></div>"""

    html = f"""<!DOCTYPE html><html dir="rtl" lang="ar"><head>
<meta charset="UTF-8"><meta http-equiv="refresh" content="10">
<title>Alpaca Bot</title><style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:#0a0a0f;color:#e0e0e0;font-family:monospace;padding:16px}}
h1{{color:#00ff88;font-size:18px;margin-bottom:6px}}
h2{{color:#7878ff;font-size:13px;margin-bottom:12px}}
.sub{{color:#555;font-size:11px;margin-bottom:16px}}
.card{{background:#12121a;border:1px solid #1e1e2e;border-radius:8px;padding:14px;margin-bottom:14px}}
.row{{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #1a1a2a;font-size:12px}}
.row:last-child{{border-bottom:none}}
.label{{color:#888}}.val{{color:#fff;font-weight:bold}}
.green{{color:#00ff88}}.red{{color:#ff4466}}.yellow{{color:#ffcc00}}
.stats-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;margin-top:8px}}
.stat{{background:#0d0d18;border:1px solid #1e1e2e;border-radius:6px;padding:10px;text-align:center}}
.stat-val{{font-size:16px;font-weight:bold;color:#fff;margin-bottom:4px}}
.stat-lbl{{font-size:10px;color:#666}}
table{{width:100%;border-collapse:collapse;font-size:11px}}
th{{background:#1e1e2e;color:#7878ff;padding:6px;text-align:right;white-space:nowrap}}
...
<h2 style="margin-bottom:8px">الصفقات النشطة</h2>
{cards}
{summary_html}
<div class="card"><h2>سجل الصفقات التفصيلي ({len(trades)})</h2>{table_html}</div>
